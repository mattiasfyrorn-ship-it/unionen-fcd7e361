  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      const { data: coupleChecks } = await supabase
        .from("daily_checks")
        .select("turn_toward, turn_toward_options, gave_appreciation, climate, check_date")
        .eq("couple_id", profile.couple_id!)
        .gte("check_date", new Date(Date.now() - 7 * 86400000).toISOString().split("T")[0]);

      if (coupleChecks && coupleChecks.length > 0) {
        const ttAll = coupleChecks.filter((c) => c.turn_toward);
        if (ttAll.length > 0) {
          const pos = ttAll.filter((c) => c.turn_toward !== "missed").length;
          setOurTurnToward(Math.round((pos / ttAll.length) * 100));
        }
        setOurAppreciations(coupleChecks.filter((c) => c.gave_appreciation).length);
        const climateVals = coupleChecks.map((c) => (c as any).climate).filter((v: any) => v != null);
        if (climateVals.length > 0) {
          setOurClimate(Math.round((climateVals.reduce((a: number, b: number) => a + b, 0) / climateVals.length) * 10) / 10);
        }
      }

      // Weekly conversations count
      ...
      if (convs) {
        ...
        for (let i = 0; i < convs.length; i++) {
          ...
          if (ws.toISOString().split("T")[0] === expWeekStart.toISOString().split("T")[0]) {
            cs++;
          } else break;
        }
        setConvStreak(cs);
      ...
      const { data: repairs } = await supabase
        ...
        .select("id")
        .eq("couple_id", profile.couple_id!);
      if (repairs) setOurRepairs(repairs.length);

      // Correlation insight (14+ days)
      if (checks && checks.length >= 14) {
        // Simple: correlate nard with turn toward
          ...
          if (lowNardWeeks.length > 0 && highNardWeeks.length > 0) {
            const getWeekTT = (weekStart: string) => {
              ...
              const weekChecks = checks!.filter((c) => {
                const cd = new Date(c.check_date);
                return cd >= ws && cd < we;
              });
            ...
            if (lowTT.length > 0 && highTT.length > 0) {
              ...
              if (avgHigh > 0) {
                ...
                if (diff > 10) {
                  setInsight(`Låg NÄRD → ${diff}% färre Turn Toward.`);
                }
              }
            }
          }
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <Card className="bg-card/80 border-border/50">
            <CardContent className="p-4 flex flex-col items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">Turn Toward</span>
              <span className="text-3xl font-bold text-foreground">
                {ourTurnToward !== null ? `${ourTurnToward}%` : "–"}
              </span>
          ...
          <Card className="bg-card/80 border-border/50">
            <CardContent className="p-4 flex flex-col items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">Reparationer</span>
              <span className="text-3xl font-bold text-foreground">{ourRepairs}</span>
            </CardContent>
          </Card>
